...
final_medical_ds <- medical_ds_3[,c(
  ...
  "diag_1_enc","diag_2_enc","diag_3_enc","max_glu_serum_enc","A1Cresult_enc","change_enc",
...
training_ind <- createDataPartition(final_medical_ds_1$readmitted ,
p = 0.75,
list = FALSE,
times = 1)
...
test_set[, 26:33] <- scale(test_set[, 26:33], center = apply(training_set[, 26:33], 2, mean), scale = apply(training_set[, 26:33], 2, sd))
training_set[, 26:33] <- scale(training_set[, 26:33])
# rename the levels of the target, so they can be used as valid R variable names
# by the predict function in caret
#levels(training_set$lodgepole_pine)[levels(training_set$lodgepole_pine)==0] <- "negative"
#levels(training_set$lodgepole_pine)[levels(training_set$lodgepole_pine)==1] <- "positive"

levels(training_set$readmitted)[levels(training_set$readmitted) == '<30'] <- "Yes"
levels(training_set$readmitted)[levels(training_set$readmitted)== '>30'] <- "Yes"
levels(training_set$readmitted)[levels(training_set$readmitted)== 'NO'] <- "No"
```
...
```{r random_forest}
modelLookup("rf")
recommended_mtry <- floor(sqrt(ncol(training_set[, -1*c(34:34)])))
...
rf_onehot_model <- train(x = training_set[, -1*c(34:34)], y = training_set[, 34],
                  method = "rf",
                  tuneGrid = rfGrid,
                  trControl = rfControl,
                  importance = TRUE,
