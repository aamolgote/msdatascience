...
ds_final <- fund_stock_securities_rating_ds_final
...
#summary(training_subset_ds_final_1)
#training_subset_ds_final <- subset(training_subset_ds_final_1, select = -c(6:86))
#test_subset_ds_final <- subset(test_subset_ds_final_1, select = -c(6:86))
...
#summary(training_subset_ds_final)
#training_subset_ds_final$litigated <- as.factor(training_subset_ds_final$litigated)
#str(training_subset_ds_final[target_var_index])
summary(training_subset_ds_final)
```
...
rf_onehot_model <- train(litigated~., 
...
rocr_pred <- prediction(test_subset_ds_final$class_probabilities_onehot, test_subset_ds_final$litigated)
...
calibration_curve <- calibration(litigated ~ class_probabilities_onehot,
...
## Logistic Regression
```{r logistic_regression_medical_dataset, MESSAGE = FALSE, warning=FALSE}

logistic_regression_model <- glm(litigated ~ 
                                  cshtrd_m + 
                                  sstk + 
                                  txt + 
                                  chech +
...
```{r logistic_regression_roc_curve, MESSAGE = FALSE, warning=FALSE}
 rocr_pred <- prediction(test_subset_ds_final$prediction_target, test_subset_ds_final$litigated)
 rocr_roc <- performance(rocr_pred, measure = "tpr", x.measure = "fpr")
 plot(rocr_roc,
 colorize = TRUE,
 print.cutoffs.at = seq(0, 1, by = 0.1),
 ...
 abline(a = 0, b = 1)
```
```{r logistic_regression_auc_r, MESSAGE = FALSE, warning=FALSE}
 rocr_auc <- performance(rocr_pred, measure = "auc")
