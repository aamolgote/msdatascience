...
#fund_stock_securities_rating_ds %>%
...
```{r final_col_names_training_1, MESSAGE = FALSE, warning=FALSE}
...
```{r final_col_names_test_1, MESSAGE = FALSE, warning=FALSE}
...
```{r settlement_amount_7, MESSAGE = FALSE, warning=FALSE}
...
# trading_volumn <- ggplot(data=ds_final, aes(x=litigated, y=ao), ) + 
#   geom_boxplot() +
#   xlab("Litigated") +
#   ylab("Trading Volume - Daily") +
#   theme_minimal()
...
#grid.arrange(trading_volumn, ncol=2)
...
```{r settlement_amount_glm_1, MESSAGE = FALSE, warning=FALSE}
# settlemeny_amt_model_ds <- training_subset_ds_final %>%
#   filter(!is.na(litigation_settlement))
# settlement_amt_model_ds <- distinct(settlemeny_amt_model_ds, gvkey, .keep_all = TRUE)
# settlement_amt_training_set <- settlement_amt_model_ds[1:5,]
# settlement_amt_test_set <- settlement_amt_model_ds[6:8,]
# #settlement_amt_training_set
# #settlement_amt_test_set
# test_settlement_amt_mean <- mean(settlement_amt_test_set$litigation_settlement)
# test_settlement_amt_mse <-  mean((settlement_amt_test_set$litigation_settlement - test_settlement_amt_mean)^2)
```

```{r settlement_amount_glm_2, MESSAGE = FALSE, warning=FALSE}
# set.seed(123)
# 
# glm_settlement_amount <- glm(litigation_settlement ~ 
#               aco + acominc + ao + aoloch + aqc + bkvlps + caps + 
#               ch + chech + csho + cstk + dlc + dltt + dvt +
#               ebit + epspi + nopi + re + rect + revt + siv +
#               sstk + teq + tstk + wcap +
...
#               wc_ratio + de_ratio + roe_ratio + sp_rating_target
#               , 
#               data = settlement_amt_training_set)
# glm_settlement_predictions <- predict(glm_settlement_amount, settlement_amt_test_set)
# glm_settlement_mse <- mean((glm_settlement_predictions - settlement_amt_test_set$litigation_settlement)^2)
# glm_test_r2 <- (1 - (glm_settlement_mse/test_settlement_amt_mse))
# test_settlement_amt_mse
# glm_settlement_mse
# glm_test_r2
```
