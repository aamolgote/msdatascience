...
ggplot(data =groupedData, aes(x=ageGroup, y=totalMonthlyValue)) +
  geom_point(colour="steelblue", shape=16) 
```
...
```
```{r}
summary(customersForAnalysis$HHIncome)
...
```{r agePlots}
ggplot(data = customersForAnalysis,
       aes(x = HomeOwner, y = AverageMonthlyRevenue, group=HomeOwner)) +
  geom_boxplot()  
